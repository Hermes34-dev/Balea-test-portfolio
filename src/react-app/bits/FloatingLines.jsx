...
float wave(vec2 uv, float offset, vec2 screenUv, vec2 mouseUv, bool shouldBend) {
  ...
  if (shouldBend) {
    ...
    float bOff    = (mouseUv.y - screenUv.y) * infl * bendStrength * bendInfluence;
    y += bOff;
  }
  float m = uv.y - y;
...
void mainImage(out vec4 fragColor, in vec2 fragCoord) {
  ...
  if (enableBottom) {
    for (int i = 0; i < bottomLineCount; ++i) {
      ...
      col += lc * wave(ruv + vec2(bottomLineDistance * fi + bottomWavePosition.x, bottomWavePosition.y),
  ...
  if (enableMiddle) {
    for (int i = 0; i < middleLineCount; ++i) {
      ...
      vec2  ruv = baseUv * rotate(ang);
      col += lc * wave(ruv + vec2(middleLineDistance * fi + middleWavePosition.x, middleWavePosition.y),
  ...
  if (enableTop) {
    for (int i = 0; i < topLineCount; ++i) {
      ...
      vec2  ruv = baseUv * rotate(ang);
      ruv.x *= -1.0;
      col += lc * wave(ruv + vec2(topLineDistance * fi + topWavePosition.x, topWavePosition.y),
...
export default function FloatingLines({
  ...
  const targetInflRef     = useRef(0);
  const currentInflRef    = useRef(0);
  ...
  useEffect(() => {
    ...
    const gradientArr = Array.from({ length: MAX_GRADIENT_STOPS }, () => new Vector3(1, 1, 1));
    let gradientCount = 0;
    ...
    const onMove = e => {
      ...
      targetInflRef.current = 1.0;
      if (parallax) {
        const cx = rect.width / 2, cy = rect.height / 2;
        targetParallaxRef.current.set(
          ((e.clientX - rect.left - cx) / rect.width)   * parallaxStrength,
          (-((e.clientY - rect.top)  - cy) / rect.height) * parallaxStrength,
        );
      }
    };
    const onLeave = () => { targetInflRef.current = 0.0; };
    ...
    const loop = () => {
      ...
      if (interactive) {
        ...
        currentInflRef.current += (targetInflRef.current - currentInflRef.current) * mouseDamping;
        uniforms.bendInfluence.value = currentInflRef.current;
      }
      if (parallax) {
